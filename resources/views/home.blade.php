            ...
            <div class="col-lg-7 p-0">
                <div class="glassy full-edge">
                    <div>
                        ...
                        <p>
                            Met je <strong>i-account</strong> van <strong>curio.codes</strong> kun je inloggen op alle apps
                            van de curio software-opleidingen. Dat zijn websites die eindigen op <em>curio.codes</em>. Dit
                            account bestaat naast je <strong>edu-account</strong> account dat je van curio krijgt.
                        </p>
                    </div>
                </div>
                ...
                <div class="mt-4">
                    <div class="links-container">
                        <a target="_blank" class="button min-w-fit" href="http://student.curio.nl/">
                            Naar mijn edu-account
                            <i class="fa fa-fw fa-external-link"></i>
                        </a>
                        @foreach ($apps as $app)
                            @php $url = parse_url($app->redirect_uris[0]); @endphp

                            <a target="_blank" class="button min-w-fit" href="{{ $url['scheme'] . '://' . $url['host'] }}">
                                {{ $app->name }}
                                <i class="fa fa-fw fa-cube"></i>
                            </a>
                        @endforeach
                        @foreach ($links as $link)
                            <a target="_blank" class="button min-w-fit" href="http://curio.codes/{{ $link->short }}">
